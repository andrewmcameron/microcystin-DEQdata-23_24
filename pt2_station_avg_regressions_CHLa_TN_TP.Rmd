...
write_csv(reg.df, "data/regressionStats_2023vs2024.csv")
...
ggplot(data_chla, aes(x = CHLa23, y = CHLa24)) +
    geom_point() +
      ...
      annotate("segment", x = min(data_chla$CHLa23), 
               ...
               y = reg_chla$model_intercept + reg_chla$model_slope * min(data_chla$TP23), 
...
ggplot(data_TN, aes(x = TN23, y = TN24)) +
    geom_point() +
      ...
      annotate("segment", x = min(data_TN$TN23), 
               ...
               y = reg_TN$model_intercept + reg_TN$model_slope * min(data_TN$TP23), 
...
ggplot(data_TP, aes(x = TP23, y = TP24)) +
    geom_point() +
      ...
      annotate("segment", x = min(data_TP$TP23), 
               ...
               y = reg_TP$model_intercept + reg_TP$model_slope * min(data_TP$TP23), 
...
modSums_output2 <- list()  
  ...
  for (var_pair in list(c("CHLa", "TP"), c("CHLa", "TN"), c("TP", "TN"))) {
    ...
    modelSummary <- list(
      model = list_id,
      slope = mod.sum$coefficients[2, 1],
      MAD = mod.sum$coefficients[x, "MAD"],
      pvalue = mod.sum$coefficients[x, 4],
      intercept = mod.sum$coefficients[1, 1]
    )
...
# Create single data frame with regression statistics
 reg.df_output2 <- data.frame(model = c("CHLa vs TP", "CHLa vs TN", "TP vs TN"),
               model_slope = rep(NA, 3),
               model_MAD = rep(NA, 3),
               model_pval = rep(NA, 3),
               model_intercept = rep(NA, 3))
...
for (i in seq_along(1:3)) {
  modelSum <- modSums_output2[[reg.df_output2$model[i]]]
        ...
        reg.df_output2$model_MAD[i] <- modelSum$MAD
        reg.df_output2$model_pval[i] <-  modelSum$pvalue
        reg.df_output2$model_intercept[i] <-  modelSum$intercept
      }


write_csv(reg.df_output2, "data/regressionStats_variablePairs.csv")
...
# Calculate position for p-value: place it dynamically near the upper right, clear of the data points
  pval_x_pos <- max(chlaVsTP$TP) - .015  # Adjust the 5 to change how far left from the max Year
  pval_y_pos <- max_y * 0.95           # Place it slightly below the upper limit of the y-axis
...
ggplot(chlaVsTP, aes(x = TP, y = CHLa)) +
    geom_point() +
      ...
      # Regression line
      annotate("segment", x = min(chlaVsTP$TP), 
               xend = max(chlaVsTP$TP), 
               y = reg_chlaVsTP$model_intercept + reg_chlaVsTP$model_slope * min(chlaVsTP$TP), 
               yend = reg_chlaVsTP$model_intercept  + reg_chlaVsTP$model_slope * (max(chlaVsTP$TP)),
               color = "red", linewidth = .42, alpha = .6) +
      labs(title = NULL,
           x = "TP (\u03BCg L\u207B\u00B9)", 
           y = "CHLa (\u03BCg L\u207B\u00B9)") +
...
# Calculate position for p-value: place it dynamically near the upper right, clear of the data points
  pval_x_pos <- max(chlaVsTN$TN) - .015  # Adjust the 5 to change how far left from the max Year
  pval_y_pos <- max_y * 0.95           # Place it slightly below the upper limit of the y-axis
...
ggplot(chlaVsTN, aes(x = TN, y = CHLa)) +
    geom_point() +
      ...
      # Regression line
      annotate("segment", x = min(chlaVsTN$TN), 
               xend = max(chlaVsTN$TN), 
               y = reg_chlaVsTN$model_intercept + reg_chlaVsTN$model_slope * min(chlaVsTN$TN), 
               yend = reg_chlaVsTN$model_intercept  + reg_chlaVsTN$model_slope * (max(chlaVsTN$TN)),
               color = "red", linewidth = .42, alpha = .6) +
      labs(title = NULL,
           x = "TN (\u03BCg L\u207B\u00B9)", 
           y = "CHLa (\u03BCg L\u207B\u00B9)") +
...
# Calculate position for p-value: place it dynamically near the upper right, clear of the data points
  pval_x_pos <- max(TPVsTN$TN) - .015  # Adjust the 5 to change how far left from the max Year
  pval_y_pos <- max_y * 0.95           # Place it slightly below the upper limit of the y-axis
...
ggplot(TPVsTN, aes(x = TN, y = TP)) +
    geom_point() +
      ...
      # Regression line
      annotate("segment", x = min(TPVsTN$TN), 
               xend = max(TPVsTN$TN), 
               y = reg_TPVsTN$model_intercept + reg_TPVsTN$model_slope * min(TPVsTN$TN), 
               yend = reg_TPVsTN$model_intercept  + reg_TPVsTN$model_slope * (max(TPVsTN$TN)),
               color = "red", linewidth = .42, alpha = .6) +
      labs(title = NULL,
           x = "TN (\u03BCg L\u207B\u00B9)", 
           y = "TP (\u03BCg L\u207B\u00B9)") +
